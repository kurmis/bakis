...
ARM architekt\={u}ra paremta RISC\footnote{RISC - reduced instruction set computing.} - paprastesni\k{u} komand\k{u} komnad\k{u} sistema. Paprastai kiekviena instrukcija atliekama per vien\k{a} takt\k{a}, bei komand\k{u} atkodavimas reikalauja ma\v{z}iau tranzistori\k{u}, nei mikroprocesoriams paremtiems sud\.{e}tingesni\k{u} komand\k{u} sistema $($pvz. CISC\footnote{CISC - complex instruction set computing}$)$
